...
const DrawingCanvas = ({ drawingId, onExitRoom }) => {
    ...
    const loadDrawingState = async () => {
      try {
        const res = await axios.get(
          `http://192.168.1.10:5000/api/drawings/state/${drawingId}`
        );
        if (res.data) {
          const image = new Image();
          image.onload = () => {
  ...
  const drawShape = (context, pos, sides, size) => {
    ...
    context.fillStyle = `rgba(${parseInt(color.slice(1, 3), 16)}, ${parseInt(
      color.slice(3, 5),
      16
    )}, ${parseInt(color.slice(5, 7), 16)}, ${transparency})`;
    context.fill();
  };
  ...
  const draw = (e) => {
    ...
    context.strokeStyle = `rgba(${parseInt(color.slice(1, 3), 16)}, ${parseInt(
      color.slice(3, 5),
      16
    )}, ${parseInt(color.slice(5, 7), 16)}, ${transparency})`;
    ...
    <Box p={4}>
      ...
      >
        <Text fontSize={{ base: "md", md: "lg" }}>
          <b>Room no: {drawingId}</b>
        </Text>
        <Text fontSize={{ base: "md", md: "lg" }}>
          <b>Clients Connected: {clientCount}</b>
        </Text>
        ...
        <VStack p={0}>
          ...
          <Button color={"blue"} onClick={exitRoom}>
            Exit Room
          </Button>
        </VStack>
      </Flex>
